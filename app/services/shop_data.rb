...
class ShopData
  ...
  def self.shop_status_barchart(current_user)
    ...
    states.each do |state|
      data[state] = {name: state.humanize, color: stop_light_color_map[state], data: Array.new(regions.size, 0)}
    end
  ...
  def self.region_revenue_bar_chart(current_user)
    ...
    shops = current_user.shop_rental_boxes.where(id: payments.collect { |p| p.transactable_id } )
    regions = shops.collect { |shop| shop.region }.uniq
  ...
  def self.rental_income_timeline(current_user)
    payments = current_user.transactions.where(category: 'shop_rent')
    shops = current_user.shop_rental_boxes.where(id: payments.collect { |p| p.transactable_id } )
    regions = shops.collect { |shop| shop.region }
