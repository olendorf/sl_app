...
RSpec.describe 'Api::V1::Analyzable::EmployeesController', type: :request do
  ...
  describe 'index' do
    before(:each) do
      32.times do |i|
        employee = FactoryBot.create(:employee, hourly_pay: 10, avatar_name: "Random avie#{i}",
                                                user_id: user.id)
        4.times do
          employee.work_sessions << FactoryBot.build(:work_session, duration: 2.0,
                                                                    pay: 2.0 * employee.hourly_pay)
        end
      end
    end
  ...
  describe 'update' do
    ...
    let(:atts) { { max_hours: 60, hourly_pay: 10_000 } }
      ...
      before(:each) do
        4.times do |i|
          employee = FactoryBot.create(
            :employee, hourly_pay: 10,
                       avatar_name: "Random avie#{i}",
                       user_id: user.id
          )
          4.times do
            employee.work_sessions << FactoryBot.build(
              :work_session,
              duration: 2.0,
              pay: 2.0 * employee.hourly_pay
            )
