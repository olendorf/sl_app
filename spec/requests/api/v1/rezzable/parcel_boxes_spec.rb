  ...
  describe 'setting a parcel box out' do 
    ...
    context 'there are no other parcels on the sim' do 
      it 'should return ok status ' do 
        post path, params: atts.to_json, headers: headers(parcel_box, api_key: Settings.default.web_object.api_key)
    ...
    context 'there are other parcels on the sim' do 
      before(:each) do
        ...
        2.times do |i|
          ...
          FactoryBot.create :parcel, region: 'foo', parcel_name: "taken #{i}", parcel_box_id: parcel_box.id, user_id: user.id
        end
      end
      it 'should return ok status ' do 
        post path, params: atts.to_json, headers: headers(parcel_box, api_key: Settings.default.web_object.api_key)
